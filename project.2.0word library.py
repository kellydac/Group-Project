...
# Define a library for performance metric names
...
# Function to get the proper name of a performance metric
def get_metric_name(metric_abbr):
    return performance_metric_names.get(metric_abbr, metric_abbr)  # Return the proper name if found, otherwise return the original abbreviation
...
def plot_performance_by_age(summary_stats, performance_metric):
    ...
    plt.plot(summary_stats['age_group'], summary_stats[f'{performance_metric}_mean'], marker='o', label='Mean')
    ...
    plt.ylabel(get_metric_name(performance_metric))
    plt.title(f'{get_metric_name(performance_metric)} by Age Group')
    plt.legend()
    plt.grid(True)
