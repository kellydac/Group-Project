...
def categorize_age(df):
    ...
    bins = [20, 24, 28, 32, 36, 38]
    labels = ['20-24', '25-28', '29-32', '33-36', '37-38']
...
def calculate_summary_statistics(df, performance_metrics):
    # Filter out data where performance metrics are greater than 0
    filtered_df = df[df[performance_metrics] > 0]
    
    # Group filtered data by age group and calculate mean, median, and standard deviation of performance metrics
    summary_stats = filtered_df.groupby('age_group')[performance_metrics].agg(['mean', 'median', 'std']).reset_index()
    
    # Find the age range representing peak performance for the performance metric
    peak_age_range = summary_stats.loc[summary_stats['mean'].idxmax(), 'age_group']
    print(f"The age range representing peak performance for {performance_metrics} is: {peak_age_range} years old")
...
def main():
    ...
    pitching_performance_metrics = ['ERA', 'IPouts', 'SO']
