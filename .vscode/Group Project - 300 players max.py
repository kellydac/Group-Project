...
folder_path = r'C:\Users\kelly\Downloads'
...
def calculate_age(df):
    ...
    # Calculate player age by subtracting birthYear from yearID
    df['age'] = (df['yearID'] - df['birthYear']).dt.days // 365
    
    return df

# Define function to group players into age categories with maximum 300 players per group
def categorize_age(df):
    # Define age categories
    bins = [0, 24, 29, 34, 39, 44, 49, 54, 59, 64, 69, 74, 79, 84, 89, 94, 99]
    labels = ['0-24', '25-29', '30-34', '35-39', '40-44', '45-49', '50-54', '55-59', '60-64', '65-69', '70-74', '75-79', '80-84', '85-89', '90-94', '95-99']
    
    # Categorize age using cut function and limit each group to 300 players
    df['age_group'] = pd.cut(df['age'], bins=bins, labels=labels, right=False)
    df['rank'] = df.groupby('age_group')['age'].rank(method='first')
    df = df[df['rank'] <= 300]
    
    return df

# Define function to calculate summary statistics for performance metrics by age category
def calculate_summary_statistics(df, performance_metric):
    # Group data by age group and calculate mean, median, and standard deviation of the performance metric
    summary_stats = df.groupby('age_group')[performance_metric].agg(['mean', 'median', 'std']).reset_index()
...
def main():
    ...
    # Categorize player age into age groups with maximum 300 players per group
    batting_df = categorize_age(batting_df)
    pitching_df = categorize_age(pitching_df)
    
    # List of performance metrics to analyze
    batting_performance_metrics = ['G_batting', 'AB', 'R', 'H', '2B', '3B', 'HR', 'RBI', 'SB', 'CS', 'BB', 'SO', 'IBB', 'HBP', 'SH', 'SF', 'GIDP']
    pitching_performance_metrics = ['W', 'L', 'G', 'GS', 'CG', 'SHO', 'SV', 'IPOuts', 'H', 'ER', 'HR', 'BB', 'SO', 'BAOpp', 'ERA', 'IBB', 'WP', 'HBP', 'BK', 'BFP', 'GF', 'R', 'SH', 'SF', 'GIDP']
